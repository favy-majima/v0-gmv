...
import type { StoreDetailData, DashboardData, StoreDetailPeriod, ProductData, ProductCombination } from "@/lib/spreadsheet"
...
export default function ShopDetailPage() {
  ...
  const { data: dashboardData } = useSWR<DashboardData>("/api/dashboard", fetcher)
  ...
  const stores = dashboardData?.thisMonth?.stores || []

  // 選択された期間に応じたデータを取得
  const currentPeriodData = useMemo((): StoreDetailPeriod => {
    if (!storeData) {
      return {
        summary: { totalSales: 0, customers: 0, averagePerCustomer: 0 },
        products: [],
        combinations: [],
      }
    }
  ...
  // 組み合わせにおける商品別登場回数（フィルター適用）
  const productFrequency = useMemo(() => {
    const freqMap = new Map<string, { appearances: number; totalCombinations: number; totalSales: number }>()
    for (const combo of currentPeriodData.combinations) {
      const count = getFilteredValue(combo.count, combo.prepaidCount, combo.postpaidCount, paymentFilter)
      const sales = getFilteredValue(combo.totalSales, combo.prepaidSales, combo.postpaidSales, paymentFilter)
      for (const product of combo.products) {
        const existing = freqMap.get(product) || { appearances: 0, totalCombinations: 0, totalSales: 0 }
        freqMap.set(product, {
          appearances: existing.appearances + count,
          totalCombinations: existing.totalCombinations + 1,
  ...
  }, [currentPeriodData.combinations, paymentFilter])
